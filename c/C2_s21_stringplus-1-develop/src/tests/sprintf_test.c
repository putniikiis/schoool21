...
START_TEST(test_one_hex_lower) {
  char str1[BUFF_SIZE];
  char str2[BUFF_SIZE];
  s21_sprintf(str1, "%x", (unsigned)INT32_MIN),
...
START_TEST(g_zero) {
  ...
  char format[] = "%.5g";
  double hex = 0;
  s21_sprintf(str1, format, hex), sprintf(str2, format, hex);
...
START_TEST(long_float) {
  long double f = 0.14;
  char str1[600];
  char str2[600];
  int a = s21_sprintf(str1, "%Lf", f);
  int b = sprintf(str2, "%Lf", f);
  ck_assert_int_eq(a, b);
}
END_TEST
